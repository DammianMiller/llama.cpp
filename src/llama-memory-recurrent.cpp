    ...
    if (0 <= seq_id) {
        ...
        if (tail_id >= 0) {
            ...
            if (0 < p0 && p0 <= cell.pos && p1 > cell.pos) {
                ...
                int32_t best_cell = -1;
                LLAMA_LOG_DEBUG("%s: partial tail removal: seq_id=%d, p0=%d, cell.pos=%d, searching for checkpoint at pos=%d\n",
                    __func__, seq_id, p0, cell.pos, p0 - 1);
                for (uint32_t i = 0; i < size; ++i) {
                    if (cells[i].has_seq_id(seq_id) && cells[i].pos == p0 - 1) {
                        LLAMA_LOG_DEBUG("%s: found checkpoint cell %d at pos %d\n", __func__, i, cells[i].pos);
                        best_cell = i;
                        break;
                    }
                }
...
void llama_memory_recurrent::copy_cell(int32_t i_src, int32_t i_dst) {
    ...
    // Bounds check to prevent tensor write out of bounds
    ...
    for (uint32_t il = 0; il < hparams.n_layer; ++il) {
        if (r_l[il]) {
            ...
            ggml_backend_tensor_get(r_l[il], buf.data(), i_src * r_row_size, r_row_size);
            ggml_backend_tensor_set(r_l[il], buf.data(), i_dst * r_row_size, r_row_size);
        }
        if (s_l[il]) {
            const size_t s_row_size = ggml_row_size(s_l[il]->type, hparams.n_embd_s());
            std::vector<uint8_t> buf(s_row_size);
            ggml_backend_tensor_get(s_l[il], buf.data(), i_src * s_row_size, s_row_size);
            ggml_backend_tensor_set(s_l[il], buf.data(), i_dst * s_row_size, s_row_size);
        }
    }
}
...
bool llama_memory_recurrent::find_slot(const llama_ubatch & ubatch) {
    ...
    for (uint32_t s = 0; s < n_seqs; ++s) {
        ...
        if (!has_cell) {
            GGML_ASSERT(next_empty_cell < size && "no empty cell available for new sequence");
            auto & empty_cell = cells[next_empty_cell];
            ...
            LLAMA_LOG_DEBUG("%s: has_cell=true path for seq_id=%d, cur_tail=%d, pos=%d, next_empty_cell=%u, is_empty=%d, used=%u, size=%u\n",
                __func__, seq_id, cur_tail, cells[cur_tail].pos, next_empty_cell,
                (next_empty_cell < size) ? cells[next_empty_cell].is_empty() : -1, used, size);

            // Only create checkpoints when evaluating multi-token batches (speculative decoding).
            // Single-token generation doesn't need checkpoints and the copy overhead is significant.
            const bool is_speculative_batch = (n_seq_tokens > 1);
            bool has_empty_cell = (next_empty_cell < size && cells[next_empty_cell].is_empty());
            bool can_checkpoint = is_speculative_batch && has_empty_cell && (get_cell_count(seq_id) < 8 && used < size * 0.9);

            if (!can_checkpoint) {
                // No empty cell available or too many checkpoints -- evict the oldest checkpoint
                int32_t oldest = -1;
                llama_pos min_pos = std::numeric_limits<llama_pos>::max();
                for (uint32_t j = 0; j < size; ++j) {
                ...
                if (oldest >= 0) {
                    ...
                    can_checkpoint = true;
                    // After eviction, find the newly freed cell
                    if (!has_empty_cell) {
            ...
            if (can_checkpoint && has_empty_cell && next_empty_cell < size) {
